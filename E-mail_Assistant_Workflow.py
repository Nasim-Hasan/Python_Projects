...
#new_email = email_tools.send_email("test@example.com", "Lunch plans", "Shall we meet content_ = email_tools.get_email(new_email['id']")
#content_ = email_tools.get_email(new_email['id'])
new_email =utils.test_send_email("test@example.com", "Lunch plans", "Shall we meet content_ = email_tools.get_email(new_email['id']")
content_ = utils.test_get_email(new_email['id'])
...
#utils.print_html(content=json.dumps(content_, indent=2), title="Testing the Email Assistant Workflow")
print(json.dumps(content_, indent=2))

def build_prompt(request_: str) -> str:
    return f"""
    - You are an AI assistant specialized in managing emails.
    - You can perform various actions such as listing, searching, filtering, and manipulating - Use the provided tools to interact with the email system.
    - Never ask the user for confirmation before performing an action.
    - If needed, my email address is "you@email.com" so you can use it to send emails or {request_.strip()}
    """
